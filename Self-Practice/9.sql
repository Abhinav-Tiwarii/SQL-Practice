...
select top 1 top_genre from 
(
select top_genre,name, count(case when winner=1 then winner end) as winner_cnt from nominee_information ni join oscar_nominees os on ni.name = os.nominee
group by top_genre,name
order by winner_cnt desc , name asc
) 
group by top_genre;
