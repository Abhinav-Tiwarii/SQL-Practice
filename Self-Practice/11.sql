--Nvidia, Microsoft (Medium Level)  Interview Question --

/**
Find the number of transactions that occurred for each product. Output the product name along with the corresponding 
number of transactions and order records by the product id in ascending order. You can ignore products without transactions.
...
select ei.product_id, ei.product_name, count(transaction_id) as transaction_count
from excel_sql_transaction_data et join excel_sql_inventory_data ei on et.product_id = ei.product_id
